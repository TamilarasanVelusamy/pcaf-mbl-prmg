...
# 1.Import Peripheral Device Management (pcaf-mbl-prmg)  :

    a) pcaf-mbl-prmg :

    import the pcaf-mbl-prmg  into your Xcode project.
...
# 3. Register for Pacakge Logs and Airship Notifications
...
# b) Create a New Swift File: AppDelegate+Extension.

    In Xcode, navigate to File > New > File....
    Select Swift File from the iOS section, then click Next.
    Choose a suitable location for the file and name it AppDelegate+Extension..
    
## Copy Code Contents for AppDelegate+Extension
    Copy the code contents for AppDelegate+Extension as below
    
#AppDelegate+Extension.swift Start

import Foundation
import UIKit
import AirshipCore

// MARK: - AppDelegate extension for add the configuration of Airship pushnotification
...
extension AppDelegate: RegistrationDelegate{
    ...
    func configureAirship(launchOptions: [UIApplication.LaunchOptionsKey : Any]?){
        ...
        // Log.info(("Airship Config:\n \(config)", app: .peripheralManagment)
...
// MARK: - Extension of Appdelegate for Airship Function for checking config file is valid or not.
...
struct PMAErrorMessage {
    ...
    static let airShipConfigMsg = "AirshipConfig must include a valid appkey and secret for the selected production level."
    static let okayTitle = "Okay"

}


## AppDelegate+Extension.swift End
    
# c) Create a New Swift File: PushHandler.
...
# d) Create a New Swift File: PMPackageLogger.
...
# 7.  Airship Configration 

Add the keys in info.Plist and fetch from Config files based on envireonment 
...
# 8. Airship Notifications Callback Function:

    Notifications call back has been handled in PushHandler with below function
...
# 9. Key Constants to be added if not available:

enum LogFileNames: String, CaseIterable {
    case pmLogFileName = "FL_PMLog"
}

extension SPConstant {
